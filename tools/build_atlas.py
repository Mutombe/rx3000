...
def hints(rel: str) -> list[str]:
    """The tooltips a screen already shows, which are how it teaches itself.

    "Double-click an amount to set it. It needs a code." is on the dispensary's
    Amount column right now. It is the exact answer to "how do I set a price on
    a script", written by whoever built the column, and until this was
    collected the assistant had to guess at it and did.
    """
    if not rel:
        return []
    src = _read(rel if rel.endswith(".tsx") else rel + ".tsx")
    said = re.findall(r'title="([^"{}]{12,120})"', src)
    seen, out = set(), []
    for hint in said:
...
# ------------------------------------------------------------------ the keys
def page_keys(rel: str) -> list[dict]:
    """The keys a page binds itself.

    These win over keymap.ts, and they have to. keymap.ts is the reference list
    copied from the incumbent; the dispensary binds its own, and the two have
    drifted: the reference says F2 is Ointment and F3 marks a line as cash,
    while the screen actually uses F2 for Find patient and F3 for Add medicine.
    An assistant reading the stale list would teach the wrong keys with total
    confidence, which is the exact failure this whole map exists to prevent.
    """
    if not rel:
        return []
    src = _read(rel if rel.endswith(".tsx") else rel + ".tsx")
    out = []
    for combo, label in re.findall(
            r'\{\s*combo:\s*"([^"]+)",\s*label:\s*"([^"]+)"', src):
        group = re.search(r'combo:\s*"%s",\s*label:\s*"%s",\s*group:\s*"([^"]+)"'
                          % (re.escape(combo), re.escape(label)), src)
        out.append({"combo": combo, "label": label,
...
def build() -> dict:
    ...
    return {
        ...
        "screens": screens,
        # The reference list, minus anything a screen binds for itself. What a
        # screen does beats what the reference says it should do.
        "keys": [k for k in keys()
                 if not any(k["combo"] == p["combo"]
                            for s in screens for p in s["keys"])],
    }
...
if __name__ == "__main__":
    ...
    hinted = sum(len(s["hints"]) for s in atlas["screens"])
    bound = sum(len(s["keys"]) for s in atlas["screens"])
    ...
    print(f"{hinted} tooltips collected, which is how a screen teaches itself")
    print(f"{bound} keys bound by a screen, {len(atlas['keys'])} more in the reference list")
